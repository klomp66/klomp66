...
logFileName = 'C:\\Interfaces\\AccessGUDID\\AccessGUDID_Logger.log'
...
writeLog.info("Starting Access GUDID download and processing.")
        ...
        for xmlFile in xmlFileList:
            ...
            for child in root:
                ...
                if actualTag(child.tag) == "device":
                    ...
                    companyName = _getXPathText(child, 'companyName')
                    versionModelNumber = _getXPathText(child, "versionModelNumber")
                    deviceId = _getXPathText(child, "deviceId", ["identifiers", "identifier"])

                    checkKeyVal = "%s|%s" % (companyName, versionModelNumber)

                    # Check the key Map
                    match = replicationKeyMap.get(checkKeyVal, "Not Found")
                    if match == "Not Found":
                        match2 = replicationKeyMap.get(deviceId, "Not Found")
                        if match2 == "Not Found":
                            writeLog.info("No Match could be found for %s - %s" % (checkKeyVal, deviceId))
                            continue
                        else:
                            writeLog.info("Match Found based on GTIN! %s" % (deviceId))
                            delimitedLine = _generateDelimLine(child)
                            writeLog.info("Delimited Line generated...")
                            writeFile.write(delimitedLine)
                    else:
                        writeLog.info("Match Found based on Mfg Name / Part #! %s" % (checkKeyVal))
                        delimitedLine = _generateDelimLine(child)
                        writeLog.info("Delimited Line generated...")
                        writeFile.write(delimitedLine)
...
try:
    # Start by loading in the Mayo KeyVal file and create a dictionary of keys.
    MAYO_KEYVAL_FILE = 'C:\\Interfaces\\AccessGUDID\\MayoReplicationKeys.txt'
    replicationKeyMap = _createReplicationKeyMap(MAYO_KEYVAL_FILE)
    ...
    if "Daily" in VERSION:
        ...
        if replicationKeyMap != None:
            # Now let's start with the daily file, to keep the barf to a minimum, till we get this all working.
    ...
    if "Full" in VERSION:
        ...
        if replicationKeyMap != None:
            # Now we can move on to the giant file, see if we find any matches at all!
    ...
    writeLog.info("Processing Complete.")
